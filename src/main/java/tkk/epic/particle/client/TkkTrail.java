...
@OnlyIn(Dist.CLIENT)
public class TkkTrail extends TextureSheetParticle {
    ...
    protected final List<TkkTrail.TrailEdge> invisibleTrailEdges;
    protected final List<TkkTrail.TrailEdge> visibleTrailEdges;
    protected boolean animationEnd;
    protected float startEdgeCorrection = 0.0F;

    protected TkkTrail(ClientLevel level, LivingEntityPatch<?> entitypatch, Joint joint, StaticAnimation animation, TrailInfo trailInfo, SpriteSet spriteSet) {
        super(level, 0, 0, 0);
        ...
        this.invisibleTrailEdges.add(new TkkTrail.TrailEdge(prevStartPos, prevEndPos, this.trailInfo.trailLifetime));
        this.invisibleTrailEdges.add(new TkkTrail.TrailEdge(middleStartPos, middleEndPos, this.trailInfo.trailLifetime));
        this.invisibleTrailEdges.add(new TkkTrail.TrailEdge(currentStartPos, currentEndPos, this.trailInfo.trailLifetime));
    ...
    protected TkkTrail(Armature armature, LivingEntityPatch<?> entitypatch, Joint joint, StaticAnimation animation, TrailInfo trailInfo) {
        super(null, 0, 0, 0);
    ...
    public void tick() {
        ...
        if (isTrailInvisible) {
            ...
            TkkTrail.TrailEdge edge1;
            TkkTrail.TrailEdge edge2;

            if (isFirstTrail) {
                int lastIdx = this.invisibleTrailEdges.size() - 1;
                edge1 = this.invisibleTrailEdges.get(lastIdx);
                edge2 = new TkkTrail.TrailEdge(prevStartPos, prevEndPos, -1);
            } else {
    ...
    public void render(VertexConsumer vertexConsumer, Camera camera, float partialTick) {
        ...
        for (int i = (int)(startEdge); i < (int)endEdge + 1; i++) {
            TkkTrail.TrailEdge e1 = this.visibleTrailEdges.get(i);
            TkkTrail.TrailEdge e2 = this.visibleTrailEdges.get(i + 1);
    ...
    protected void makeTrailEdges(List<Vec3> startPositions, List<Vec3> endPositions, List<TkkTrail.TrailEdge> dest) {
        for (int i = 0; i < startPositions.size(); i++) {
            dest.add(new TkkTrail.TrailEdge(startPositions.get(i), endPositions.get(i), this.trailInfo.trailLifetime));
        }
    }
    ...
    public static class Provider implements ParticleProvider<SimpleParticleType> {
        ...
        public Particle createParticle(SimpleParticleType typeIn, ClientLevel level, double x, double y, double z, double xSpeed, double ySpeed, double zSpeed) {
            ...
            if (entity != null) {
                ...
                tkk.epic.capability.epicAdd.trail.TkkTrail tkkTrail = tkkCustomTrail.readyTrails.get(idx);
                ...
                if (entitypatch != null && animation != null) {
                    return new TkkTrail(level, entitypatch, entitypatch.getArmature().searchJointById(jointId), animation, trailInfo, this.spriteSeta);
                }
            }
