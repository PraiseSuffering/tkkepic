...
@OnlyIn(Dist.CLIENT)
public class VanillaTrailParticle extends TextureSheetParticle {
    ...
    protected final List<VanillaTrailParticle.TrailEdge> invisibleTrailEdges;
    protected final List<VanillaTrailParticle.TrailEdge> visibleTrailEdges;
    ...
    protected VanillaTrailParticle(ClientLevel level, LivingEntityPatch<?> entitypatch, Joint joint, StaticAnimation animation, ParticleTrail trailInfo, SpriteSet spriteSet, ItemSkin itemSkin) {
        super(level, 0, 0, 0);
        ...
        this.invisibleTrailEdges.add(new VanillaTrailParticle.TrailEdge(prevStartPos, prevEndPos, this.trailInfo.trailLifetime));
        this.invisibleTrailEdges.add(new VanillaTrailParticle.TrailEdge(middleStartPos, middleEndPos, this.trailInfo.trailLifetime));
        this.invisibleTrailEdges.add(new VanillaTrailParticle.TrailEdge(currentStartPos, currentEndPos, this.trailInfo.trailLifetime));
        /*
    ...
    protected VanillaTrailParticle(Armature armature, LivingEntityPatch<?> entitypatch, Joint joint, StaticAnimation animation, ParticleTrail trailInfo, ItemSkin itemSkin) {
        super(null, 0, 0, 0);
        this.itemSkin=itemSkin;
        this.entitypatch = entitypatch;
        this.joint = joint;
    ...
    public void tick() {
        ...
        if (isTrailInvisible) {
            ...
            VanillaTrailParticle.TrailEdge edge1;
            VanillaTrailParticle.TrailEdge edge2;

            if (isFirstTrail) {
                int lastIdx = this.invisibleTrailEdges.size() - 1;
                edge1 = this.invisibleTrailEdges.get(lastIdx);
                edge2 = new VanillaTrailParticle.TrailEdge(prevStartPos, prevEndPos, -1);
            } else {
    ...
    protected void makeTrailEdges(List<Vec3> startPositions, List<Vec3> endPositions, List<VanillaTrailParticle.TrailEdge> dest) {
        for (int i = 0; i < startPositions.size(); i++) {
            dest.add(new VanillaTrailParticle.TrailEdge(startPositions.get(i), endPositions.get(i), this.trailInfo.trailLifetime));
        }
    }
            ...
            try {
                ...
                if (entity != null) {
                    ...
                    if (entitypatch != null && animation != null && trailInfo.isPresent()) {
                        VanillaTrailParticleDataLoader.VanillaParticleTrailData data=VanillaTrailParticleDataLoader.PARTICLE_DATA.get(result.texturePath);
                        if(data==null){
                            TkkEpic.LOGGER.log(org.apache.logging.log4j.Level.ERROR,"can not find VanillaParticleTrailData,example tkkepic:example_flame,tkkepic:example_dust_red");
                            return null;
                        }
                        ParticleTrail particleTrail = new ParticleTrail(result.start, result.end, entitypatch.getArmature().searchJointById(jointId).getName(), result.startTime, result.endTime, result.fadeTime, result.interpolateCount, result.trailLifetime, result.hand == InteractionHand.MAIN_HAND, false, data.particleId,data.particleArgs,data.spaceBetween,data.speed,data.dist,data.count);
                        return new VanillaTrailParticle(level, entitypatch, entitypatch.getArmature().searchJointById(jointId), animation, particleTrail, this.spriteSet, itemSkin);
                    }
                }
            }catch (Exception e){
                TkkEpic.getInstance().broadcast("vanilla_particle_trail error");
                TkkEpic.LOGGER.error(e);
            }
            return null;
        }
    }
